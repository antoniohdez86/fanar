$(function(){
	...
	$dialog1_Acept.bind('click', function() {
		...
		var ajax_params = "mode=" + $mode + "&id=" + $text.eq(0).val() + "&nombre=" + $text.eq(1).val() + "&ape1=" + $text.eq(2).val() + "&ape2=" + $text.eq(3).val() + "&dir=" + $text.eq(4).val() + "&prof=" + $text.eq(5).val() + "&tel=" + $text.eq(6).val() + "&user=" + $text.eq(7).val() + "&pass=" + $text.eq(8).val() + "&act=" + $checkbox;
	...
	$buttons.bind('click',function(e) {
		...
		switch($this.text()) {
			...
			case "Editar":
				...
				$text.eq(7).val($cells.eq(7).text());	// USUARIO
				$text.eq(8).val($cells.eq(8).text());	// PASSWORD
				
				$text.eq(0).attr('disabled',true);
				
				$value = $cells.eq(9).text() == "SI" ? true : false;
