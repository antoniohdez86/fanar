...
$US_form_title = $('.title',$US_form);
$US_form_msg = $('.message',$US_form);
...
$US_table_msg = $('.tbl-usr-log .message');
...
} else if($US_hdn.val()=='edit') { 
$mode = 'upd'; $ie = "&ine=" + $US_txt.eq(2).val() + "&ie=" + $US_rdo.val(); msg = 'actualizando usuario...';
...
async: $async,
cache: false,
beforeSend:function(){ $US_form_msg.text(msg).show(); },
success: function(SERVER_RESPONSE) {
$US_form_msg.text('').hide();
...
$US_form_title.text('Nuevo Usuario');
$US_txt.eq(0).focus();
showDialogUs();
break; // NUEVO
case "Editar":
...
$US_form_title.text('Editar Usuario');
showDialogUs();
break; // EDITAR
case "Eliminar":
if($US_noData.parent().parent().hasClass('tbBody')) { alert('Sin registros de usuarios!!'); return; }
...
async: $async,
cache: false,
beforeSend:function() { $US_table_msg.text('Eliminando usuario...').show(); },
success: function(SERVER_RESPONSE) {
$US_table_msg.text('').hide();
if(!SERVER_RESPONSE) { alert("La respuesta del servidor esta vacia"); return; }
var DATA = SERVER_RESPONSE.split('|');
if( DATA.length != 3 ){ alert("La respuesta del servidor tienen un formato de respuesta incorrecta"); alert(": " + SERVER_RESPONSE); return; }
SERVER_CODE = DATA[0]; SERVER_MESAGE = DATA[1]; SERVER_TEST = DATA[2];
if(SERVER_TEST) {alert(SERVER_TEST); }
switch (SERVER_CODE) {
case "USER_DELETED":
