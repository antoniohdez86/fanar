$(function(){
	...
	var $tblEtapas = $('.tbl-etapas tbody').eq(2),
	$tblEtapasRows = ('tr', $tblEtapas);

	/*****  inicializa tabla etapas  *****/
	$tblEtapasRows.bind('click',function(){
		var $cells = $('td', $(this));
		$datos_input.eq(5).val($cells.eq(1));
		$datos_input.eq(6).val($cells.eq(2));
	});
	
	/************************    TESTING     ************************/
	
	//alert($datos_input.length)
	
	//alert($tblEtapasRows.length);
	alert($tblEtapasRows.length);
