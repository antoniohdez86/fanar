$(function(){
	...
	var $DATSEG_form_hid = $('input:[type=hidden]', $DATSEG_form);
	var $DATSEG_form_ie = null;	// hidden: id empleado
	var $DATSEG_form_im = null;	// id municipio (uso futuro)

	//alert($DATSEG_form_hid.attr('name'));
	//alert($DATSEG_form_hid.val());
	...
	var $ACT_form_msg = $('.message',$ACT_form);	// message
	...
	$SEG_noData = $('.no-data',$SEG_body1);// tr no-data
	...
	$ACT_form_btn.bind('click', function(e) {
		e.preventDefault();
		if($IE!=$DATSEG_form_ie.val()){
			alert("Solo el visitante a cargo de este nucleo puede realizar seguimientos");
			return;
		}
		var $BUTTON = $(this);
		switch($BUTTON.val()) {
			case "Agregar": 
				...
				if($ETAPA_row==null){ alert('Seleccione una etapa'); return; }
				$firstEta = getNextEtapaValid();
				if($SEG_noData.parent().hasClass('tbBody')) {
					if(parseInt($ETAPA_ie.val()) > parseInt($firstEta)){ alert("No puede saltarse etapas.\nEmpiece con la primera etapa valida"); return; }
				} else {
					if(parseInt($ETAPA_ie.val()) > parseInt($firstEta)){ alert("No puede saltarse etapas.\nSolo puede seleccionar la etapa posterior a la ultima registrada"); return; }
				}
				...
				$.ajax({
					...
					cache: false,
					beforeSend: function() {
						$ACT_form_msg.text('Registrando seguimiento...');
						$ACT_form_msg.show();
					},
					success: function(SERVER_RESPONSE) {
						$ACT_form_msg.text('').hide();
			...
			case "Guardar Cambios":
				...
				$.ajax({
					...
					data: ajax_params,
					async:$async,
					beforeSend:function(){ $ACT_form_msg.text('actualizando seguimiento...').show(); },
					cache: false,
					success: function(SERVER_RESPONSE) {
						$ACT_form_msg.text('').hide();
						if(!SERVER_RESPONSE) {
	...
	function getNextEtapaValid(){
		var $tmp_E = $ETAPA_rows_enabled.eq(0);
		var $tmp_iE = $('input:hidden', $tmp_E);
		return $tmp_iE.val();
	}
	// botones EDITAR/ELIMINAR un SEGUIMIENTO
	$SEG_btns.bind('click', function(e) {
		e.preventDefault();
		if($IE!=$DATSEG_form_ie.val()){ alert("Solo el visitante a cargo de este nucleo puede realizar modificaciones a los seguimientos"); return; }
		$this = $(this);
		$mod = null;
		if($ACT_form_md.val()=='edit'){ return; }
		if(!$IN) return;	// sino se ha definido ningun nucleo entonces significa que no hay seguimientos cargados, asi q no tiene caso hacer mas comprobaciones
		if($SEG_noData.parent().hasClass('tbBody')){ alert('El nucleo no tienen ningun seguimiento'); return; }
		$SEG_row = $('tr.selected',$SEG_body1);	// obtiene fila seleccionada
		if(!$SEG_row.length){ alert('Seleccione un seguimiento'); return; } // si no hay ninguna fila seleccionada...
		...
		switch($this.text()) {
			case "Editar":
				...
				$.ajax({
					...
					data: ajax_params,
					beforeSend: function() {
						$ACT_form_msg.text('cargando datos del seguimiento...');
						$ACT_form_msg.show();
					},
					cache: false,
					success: function(SERVER_RESPONSE) {
				...
				break; // EDITAR
			case "Eliminar":
				if($SEG_row.index() < $SEG_rows.length-1){
					alert("Solo puede eliminar el ultimo seguimiento");
					return;
				}
				...
				$.ajax({
					...
					async:$async,
					beforeSend:function(){
						$ACT_form_msg.text('eliminando seguimiento...');
						$ACT_form_msg.show();
					},
					cache: false,
					success: function(SERVER_RESPONSE) {
	...
	function enableEtapas(value){
		$ETAPA_table_enabled = value;
		if(value){
			// ...
		} else {	$ETAPA_rows.removeClass('selected'); }
	}
			...
			success: function(SERVER_RESPONSE) {
				...
				switch (SERVER_CODE) {
					case "OK_DAT":
						...
						$DATSEG_form_ie = $('input:hidden', $td.eq(0));		// input: id de empleado
						$DATSEG_form_hid.eq(0).val($DATSEG_form_ie.val());	// input: id de empleado
