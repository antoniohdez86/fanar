	...
	$buttons.bind('click',function(e) {
		...
		switch($this.text()) {
			...
			case "Nuevo":
				...
				$dialog1_y = ($win_h/2) - ($dialog1_h/2);
				$dialog1.css({'left':$dialog1_x,'top':$dialog1_y});
				$('th',$dialog1).text('Nuevo Municipio');
			...
			case "Editar":
				...
				$('th',$dialog1).text('Editar Municipio');
					...
					success: function(SERVER_RESPONSE) {
						...
						switch (SERVER_CODE) {
							case "MUNIC_DELETED":
								...
								$tbody1_rows = $('tr', $tbody1);
								if(!$tbody1_rows.length){
									$noData = $("<tr class='no-data'><td colspan=5>no hay municipios registrados</td></tr>")
									$tbody1.append($noData);
								}
								break;
							default:
								alert(SERVER_CODE + "\n" + SERVER_MESAGE);
