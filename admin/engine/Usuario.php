...
class Usuario extends MyObject {
	...
	function getInfoUserOf($username) {
		
		$this->clearErrorLogs();
		
		if(!$username){
			$this->error = true;
			$this->errorCode = "EMPTYUSER";
			$this->messageError = "nombre de usuario vacio";
			return false;
		}
		...
		$sql = "SELECT aj_usuarios.usuario as user, 
							aj_usuarios.idEmpleado as iu, 
							aj_usuarios.isAdmin as ia,
							(SELECT CONCAT(aj_empleados.nombre,' ',aj_empleados.apePat,' ',aj_empleados.apeMat) FROM aj_empleados WHERE aj_empleados.idEmpleado=aj_usuarios.idEmpleado) AS empl, 
							aj_empleados.activo AS isact 
				    FROM aj_empleados, aj_usuarios
					WHERE aj_empleados.idEmpleado=aj_usuarios.idEmpleado AND aj_usuarios.usuario='".$username."'";
		...
		if(!$dt->NumRows){
			...
			$this->messageError = "No existe ningun usuario [".$username."]";
			return false;
		}
