...
//				 		 (SELECT COUNT(*) FROM aj_empleado_nucleos WHERE aj_empleado_nucleos.idEmpleado=aj_empleados.idEmpleado) = 3 ";
		...
		if($dt->NumRows == 0) 
			...
			while($empleado = $dt->Next()) 
			{
				if($empleado['nucleos']<3){
					$select .= "<option value='".$empleado['ID']."'>".$empleado['Nombre']." ".$empleado['Apell. Pat.']." ".$empleado['Apell. Mat.']."</option>";
					//$select .= "<option value='".$empleado['ID']."'>".$empleado['Nombre']." ".$empleado['Apell. Pat.']." ".$empleado['Apell. Mat.']." : ".$empleado['nucleos']."</option>";
				}
			}
