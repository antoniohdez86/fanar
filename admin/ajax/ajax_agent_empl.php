...
if($mode == 'upd') {
	...
	$empl->getInfo($EMPL_ID);
	
	if($empl->error){
		$SERVER_MESSAGE = $empl->messageError;
		$SERVER_CODE_RESPONSE = $empl->errorCode;
	} else {
		if($EMPL_ACT=='false' && $empl->isAdmin) {
			$SERVER_MESSAGE = "No esta permitido desactivar al administrador del sistema";
			$SERVER_CODE_RESPONSE = 'ERRIA';
		} else {
...
if($mode == 'del') {
	...
	if($empl->error) {
		...
		if($empl->isAdmin){
			$SERVER_MESSAGE = 'No esta permitido eliminar al administrador del sistema';
			$SERVER_CODE_RESPONSE = 'ERRIA';
		} else {
