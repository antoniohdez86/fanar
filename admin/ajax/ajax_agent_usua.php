...
// - - - - - - - - - - - - - - - - - - - - - - - - - - 	LIBRERIAS NECESARIAS
	...
	include_once('../engine/Empleado.php');		// QUITAR	
// - - - - - - - - - - - - - - - - - - - - - - - - - -
	$mode = $_POST['mode'];
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if($mode == 'ins') {
	...
	$us = new Usuario();
	$us->insertUsuario($EMPL_USER, $EMPL_PASS, $EMPL_ID, $EMPL_ISADMIN);		// crea nuevo Usuario para el empleado de id especifico
	...
	if($us->error) {
		...
		$SERVER_MESSAGE = 
			'<tr>
				...
				<td><a href="#">Edit</a></td>
				<td><a href="#">Elim</a></td>
				<td><a href="#"><input type="radio" name="idUsr" value="'.$EMPL_ID.'"></a></td>
			</tr>';
	}
...
if($mode == 'del') {
	...
	if($EMPL_USER=='admin'){
		...
		if($us->error) { 
			$SERVER_MESSAGE = "Detalle: ".$us->messageError;
			$SERVER_CODE_RESPONSE = 'ERROR_USER_OBJECT';
		} else {
			$SERVER_MESSAGE = '';
